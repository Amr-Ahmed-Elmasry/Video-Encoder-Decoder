function [readyForTransmittion,dict,quant_level,L_Stream, frameRate, L, W] = RGB_MPEG_Encode(videoSrc,quant_level, numOfFrames)
switch nargin
    case 0
...
for i = 1 : sz
    %encoder
    current_rgb = read(vidobj, i); % Get current frame
    % Loop 3 times one R, then B, then G
    for jj = 1 : 3
        current = fixLW(double(current_rgb(:,:,jj)), 8); % Change it into Gray
        [L, W] = size(current);
...
% 2. construct dict
dict = huffmandict(elements,proba);
