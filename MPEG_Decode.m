function outputMatrix = MPEG_Decode(code,dict,quant_level,L_Stream, fps, L, W, outputName)
...
for i = 1 : 2 : length(L_Stream)
    ...
    if (rem(counter - 1, 10) == 0)
        ...
        if(outputName ~= "")
            writeVideo(outputVideo,uint8(reference_back));
        end
        if(i == 1)
            outputMatrix(:,:,end) = uint8(reference_back);
        else
            outputMatrix(:,:,end+1) = uint8(reference_back);
        end
    else
        ...
        if(outputName ~= "")
            writeVideo(outputVideo,uint8(frame_back));
        end
        outputMatrix(:,:,end+1) = uint8(frame_back);
    end
    counter = counter + 1;
end
